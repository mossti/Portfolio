# Structured Latent Spaces for Assistive Robot Teleoperation
...
![3D PCA projection comparing four generative model architectures on tabletop control data vs. real body-motion prompt data — a baseline VAE, GAN, standard SSVAE, and the custom SSVAE](assets/latent-space-comparison.png)
...
![Architecture diagram showing the encoder, reparameterization, six per-class sub-classifiers feeding a meta-classifier, decoder, and the full set of geometric and temporal loss terms applied during training](assets/architecture-diagram.png)
...
![Bar chart of cosine alignment scores across all compared model types with significance annotations](assets/cosine-alignment-results.png)
...
![Combined latent space structure analysis showing near-diagonal variance decomposition, bipolar temporal trajectories, and orthogonal pairwise dimension scatter for the custom SSVAE](assets/latent-structure-analysis.png)
...
![Example of a single motion prompt enactment showing the first principal component of quaternion sensor data over time, with detected rest position, zero crossings, and rest/movement thresholds annotated](assets/prompt-temporal-structure.png)
